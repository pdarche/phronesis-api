...
class MovesMixin(object):
    """Moves API using Oauth2"""
    ...
    def moves_request(self, path, callback, access_token=None, post_args=None, **args):
        ...
        callback = self.async_callback(self._on_moves_request, callback)
        if post_args is not None:
            self.httpclient_instance.fetch(url, method="POST", body=urllib.urlencode(post_args), callback=callback)
        else:
            self.httpclient_instance.fetch(url, callback=callback)
    ...
    def _on_moves_request(self, callback, response):
        ...
        if response.error:
            logging.warning(
                "Moves Error %r" % response_body
                # "Moves Error(%s) :: Detail: %s, Message: %s, URL: %s",
                # response.error, response_body["meta"]["errorDetail"], response_body["meta"]["errorMessage"], response.request.url
            )
            callback(None)
            return
